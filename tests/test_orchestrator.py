...
def test_a_signal_becomes_a_paper_order_and_a_complete_audit_trail(
    ...
    # Confidence 71 -> 5% of the 75,000 judged sleeve = 3,750; at 140 that is
    # 26 whole shares (75/25/0 allocation 2026-08-27; table 2/5/10% 2026-09-15).
    assert sent["qty"] == 26
...
def test_every_stage_of_the_trail_is_present_and_ordered(
    ...
    assert decision.gate.max_loss == Decimal("3640.00")  # 26 x 140
...
def test_the_fill_settles_against_the_gate_not_just_the_log(
    ...
    assert state.cash == START_CASH - Decimal("3640.00")
    assert state.position(("equity", "NUE")).quantity == 26
...
def test_an_order_that_terminates_unfilled_releases_its_reservation(
    ...
    assert started.gate.state.reserved_cash == Decimal("3640.00")
...
def test_a_partial_fill_is_booked_and_the_difference_is_recorded(
    ...
    assert any("filled 6 of 26" in r.message for r in trail.stage_rejections)
...
def test_the_daily_deployment_total_survives_a_restart(
    ...
    first.loop.tick()
    assert first.gate.state.deployed_today == Decimal("3640.00")

    restarted = preflight(adapter=FakeBroker(), id_factory=counter("b"), **kwargs)
    assert restarted.gate.state.deployed_today == Decimal("3640.00")
...
def test_a_model_that_obeys_the_injection_still_cannot_exceed_the_caps(
    ...
    assert result.traded
    trail = started.audit.trail(result.decision_id)
    assert trail.decision.sizing.capital == Decimal("7500.00")  # 10% of 75,000
    assert trail.decision.sizing.fraction_of_sleeve_nav == Decimal("0.10")
    assert broker.payloads[0]["qty"] == 53  # 7500 / 140, rounded down
    assert trail.decision.gate.max_loss == Decimal("7420.00")  # 53 x 140
    # The account is nowhere near emptied: the prose asked for 50% of NAV and got
    # a 7.42% bite out of cash.
...
def test_a_fractional_venue_gets_a_round_down_fractional_order(
    ...
    capital = Decimal("1500.00")  # 2% of the 75,000 judged sleeve (2026-09-15)
...
def test_a_whole_share_venue_still_rounds_to_whole_shares(
    ...
    assert broker.payloads[0]["qty"] == Decimal("12")  # 1,500 / 123.45, rounded down
