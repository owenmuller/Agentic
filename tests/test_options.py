...
def test_a_catalyst_backed_thesis_buys_the_selected_call(
    ...
    # Sized on the HALVED options table: 5% x 75k / 2 = 1,875 premium at risk
    # (75/25/0 allocation, ruling 2026-08-27; table 2/5/10%, 2026-09-15).
    ...
    # 1,875 at mid 2.50 x 100 = 7 contracts; the gate reserved exactly that.
    order = trail.decision.gate.order
    assert order["kind"] == "option_buy_to_open"
    assert order["contracts"] == 7
    assert trail.decision.gate.max_loss == Decimal("1750.00")  # 7 x 2.50 x 100
    ...
    assert started.gate.state.cash == Decimal("100000") - Decimal("1750.00")
...
def test_an_illiquid_chain_falls_back_to_equity_at_the_full_table(
    ...
    assert trail.decision.sizing.capital == Decimal("3750.00")  # FULL 5%, not 1,875
...
def test_premium_exceeding_sized_capital_falls_back_to_equity(
    ...
    """1,875 premium at risk cannot buy one 20.00-mid contract (2,000)."""
    pricey = [quote(bid="19.90", ask="20.10")]
    started, _ = wire(
        tmp_path, limits, signals_config, research_config,
...
def test_an_option_stop_fires_on_premium_at_the_equity_fraction(
    ...
    # 7 contracts: in at 2.50 x 100, out at 1.20 x 100 -> -910.00.
    assert trail.outcome.realised_pnl == Decimal("-910.00")
