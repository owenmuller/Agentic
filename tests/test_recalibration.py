...
def test_a_conviction_verdict_buys_an_option_without_a_catalyst(
    ...
    assert trail.decision.sizing.capital == Decimal("2750.00")  # 10% / 2 of 55k
    assert order["contracts"] == 11  # 2,750 at mid 2.50 x 100
...
def test_the_short_dated_pool_is_capped_inside_the_aggregate(limits):
    """5% of the 55k sleeve = 2,750 of short-dated premium. A 10-DTE buy over it
    ...
    # 10 x 2.00 x 100 = 2,000 short-dated: inside the 2,750 pool.
    ...
    # Another 1,000 short-dated would reach 3,000 > 2,750: refused, by the sub-cap.
    ...
    assert over.limit == Decimal("2750.00")
    # The same 1,000 at 60 DTE is ordinary premium: 3,000 of 11,000 aggregate.
