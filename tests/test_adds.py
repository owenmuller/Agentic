...
#: The entry: REPORT (long NUE, 71 -> 5% = 3,750 -> 26 shares at 140 = 3,640).
ENTRY_SHARES = Decimal("26")
...
def test_a_signal_on_a_held_name_is_an_add_decision_not_a_second_position(
    ...
    # Combined cap (rule 2): confidence 86 -> 10% band = 7,500; held 3,640 ->
    # headroom 3,860 x add_fraction 0.5 = 1,930 -> 13 shares at 140.
    assert position.lots[1].quantity == Decimal("13")
    assert position.quantity == ENTRY_SHARES + 13
    assert position.entry_cost == ENTRY_COST + 13 * QUOTE
    ...
    assert decision.add.headroom == Decimal("7500") - ENTRY_COST
    assert decision.sizing.capital == Decimal("1930.00")
...
def test_an_independent_family_bumps_the_combined_cap_one_band(
    ...
    """Confidence 75 is the 5% band (3,750): with the position at 3,640 that is
    110 of headroom — nothing. The insider-filings family is independent of the
...
def test_the_same_family_gets_no_bump_and_a_full_band_is_no_headroom(
    ...
    combined cap is the 2% band (1,500) and the position already holds 3,640 —
...
def test_adds_never_exceed_the_hard_cap(tmp_path, limits, signals_config, research_config):
    ...
    position = started.exits.tracked[0]
    # 7,500 - 3,640 = 3,860 -> 27 shares at 140 = 3,780; total 7,420 <= 7,500.
    assert position.entry_cost <= Decimal("7500")
    assert position.quantity == ENTRY_SHARES + 27
...
def test_a_close_resolves_every_lot_to_its_own_outcome(
    ...
    assert outcomes["dec-1"].realised_pnl == 26 * Decimal("100") - ENTRY_COST
    assert outcomes["dec-2"].realised_pnl == 13 * Decimal("100") - 13 * QUOTE
...
def test_a_merged_position_is_rebuilt_from_the_log_after_a_restart(
    ...
    assert [lot.quantity for lot in position.lots] == [ENTRY_SHARES, Decimal("13")]
...
def test_health_renders_lots_and_convergence(tmp_path, limits, signals_config, research_config):
    ...
    assert "lots: dec-1 26@140.00 (trump_posts, 71) | dec-2 13@140.00 (insider_filings, 86)" in rendered
