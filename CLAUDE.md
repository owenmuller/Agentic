...
- **Theme → ETF expression (human ruling 2026-09-15; ETF list proposed, awaiting confirmation):** a Class 1 post with no extracted ticker that matches exactly one configured policy theme (`signals.yaml theme_etf_map`: tariffs XLI, energy XLE, defense ITA, financials XLF, rates TLT, semis SMH, broad SPY, tech QQQ) is researched with the mapped ETF proposed as its instrument; the research pass may decline. Two matching themes = no mapping (Constraint #6). Decisions expressed through the ETF are tagged `theme_etf`.
